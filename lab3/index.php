...
<body style="background-color:#34b9b1;">
    <div
        style="background-color:white;margin-left:30%;padding:30px 20px;;width:40%;margin-top:10%;border-width:0;border-style:solid;border-radius:10px;">
        <?php
            $dirName = basename(__DIR__);
            $labelName = "Утга хөрвүүлэх (түүхтэй)";
            ...
            <div class="row">
                <div class="col-sm-6" class="form-group">
                    ...
                    if (isset($_POST["in"])) {
                        ...
                        $thisVal->convertType = $dropDownItems[$typeValue];
                        $thisVal->input = $inputValue;
                        $thisVal->output = $y;
                        array_push($_SESSION["history"], $thisVal);
        ...
        <div>
            <table border="double" style="width:80%;margin-left:10%;border-width:2px;background-color:#ddebf7;">
                <tr>
                    <td colspan="4">
                        Таны хөрвүүлсэн сүүлийн 10 утга(Түүх)
                    </td>
                </tr>
                <tr>
                    <td>
                ...
                <?php
                if (isset($_SESSION["history"])) {
                    $begin = count($_SESSION["history"]) - 1;
                    $end = 0;
                    for ($i = $begin; $i >= $end; $i--) {
                        echo "<tr> <td>" . ($begin - $i + 1) . "</td> ";
                        echo "<td> " . $_SESSION["history"][$i]->convertType . " </td> ";
                        echo "<td> " . $_SESSION["history"][$i]->input . " </td> ";
                        echo "<td> " . $_SESSION["history"][$i]->output . " </td> ";
                        echo "</tr>";
                    }
                }
                ?>
